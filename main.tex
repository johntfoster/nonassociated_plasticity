...
\begin{abstract}
	...
	preserves nonnegative plastic dissipation.  Thus an associated
...
internal forces are expressed through the usual stresses.  They also gave a
stress-dependent dissipation construction for Drucker--Prager plasticity
\cite[Secs.~4--5]{collins1997}.  Other convex, energy-based dissipation models
...
\cite[Secs.~3 and 8]{drumheller2000}.  Together, these results give a direct
physical interpretation of pressure-dependent plastic flow: shear
rearrangement changes the true deformation, whereas compaction or dilation
changes distension.

This paper uses Drumheller's terminology to identify the physical mechanisms
behind this thermodynamic construction at finite deformation.  For a single
solid phase, the split
...
equivalent to the classical volumetric--isochoric split.  The main result is a
clear distinction between independent and coupled mechanisms, together with an
explanation of how associated flow for the mechanisms appears non-associated
when written in terms of pressure and shear stress.
...
Let \(\bs\chi(\mbf X,t)\) map a material point \(\mbf X\) in the reference
...
\cite[Sec.~4, eq.~(37), and Sec.~8.13]{drumheller2000}.  That relation provides
...
denote the forces conjugate to \(\dot\gamma\) and \(\dot\zeta^p\), respectively.
...
\end{equation}
The coupled rate is therefore normal to the boundary in the
\((\chi_q,\chi_v)\) plane when \(p\) is held fixed.
...
\begin{equation*}
	...
	M=\mu+\beta.
\end{equation*}
The physical forces satisfy \(\chi_q=q\) and \(\chi_v=-p\).  After this
substitution, the yield surface has volumetric slope \(M=\mu+\beta\), whereas
the plastic rate in \eqref{eq:generalized_loading_conditions} has volumetric
slope \(\beta\).  Written directly in terms of pressure and equivalent shear
stress, the plastic rate therefore follows from the potential
\begin{equation}
	g(p,q)=q-\beta p,
	\qquad
	\dot{\bs\varepsilon}^{p}
...
than those approaches.  Its contribution is to assign the shear and volume rates to
true-plastic and distension factors, distinguish independent from coupled
mechanisms, and carry the same physical interpretation to finite deformation
under the Flory closure.
...
the same loading remains at its two-surface corner while compacting.  This
worked path shows the direct mechanical consequence of using separate or
coupled plastic multipliers.
...
is held fixed.  With \(\chi_q\) and \(\chi_v\) conjugate to \(\dot\gamma\)
and \(\dot\zeta^p\), the admissible-force boundary, the physical forces, and
the plastic rates are
\begin{equation*}
...
This paper develops a single-phase kinematic and thermodynamic framework from
the finite-deformation split \(\mbf F=\mbf A\bar{\mbf F}\).  The free-energy inequality separates
